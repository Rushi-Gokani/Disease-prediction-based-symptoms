...
# Copy symptomslist from app.py (lines 501-525)
...
print(f"Length of symptomslist: {len(symptomslist)}")

# Check training data
try:
    df_train = pd.read_csv('dataset/training_data.csv')
    cols = df_train.columns
    print(f"Training data columns: {cols.tolist()}")
    print(f"Length of training data columns: {len(cols)}")
    
    # Simulate main.py logic
    train_cols = cols[:-2]
    print(f"Train features columns (cols[:-2]): {train_cols.tolist()}")
    print(f"Length of train features: {len(train_cols)}")
    
    if 'prognosis' in train_cols:
        print("CRITICAL: 'prognosis' found in train features!")
    else:
        print("'prognosis' NOT found in train features.")
    
    # Check for mismatch
    train_cols_set = set(train_cols)
    symptoms_set = set(symptomslist)
    
    missing_in_symptoms = train_cols_set - symptoms_set
    missing_in_train = symptoms_set - train_cols_set
    
    print(f"Missing in symptomslist: {missing_in_symptoms}")
    print(f"Missing in train features: {missing_in_train}")
...
#     symptoms = {s: 0 for s in symptomslist}
#     # Simulate user input
#     symptoms['itching'] = 1
